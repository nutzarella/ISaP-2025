 ...
 *    with curve 25519 (we approximate this here with a generic EC curve as in the labs).
 *
    ...
    public static void main(String[] args) throws Exception {
        ...
        env.add(new Agent("alice") {
            ...
            public void task() throws Exception {
                ...
                //    protocol (Elliptic Curve variant). She creates her secret
                //    value a and computes her public value A = g^a mod p, then
                //    sends the public value A to the server.
                // ------------------------------------------------------------
                ...
                // Rebuild server's ECDH public key B from the encoding
                final PublicKey serverDhPK =
                        KeyFactory.getInstance("EC").generatePublic(new X509EncodedKeySpec(bEncoded));
        ...
        env.add(new Agent("server") {
            ...
            public void task() throws Exception {
                ...
                //    public value B = g^b mod p (EC variant). It then receives
                //    Alice's public value A and combines it with its own secret
                //    value to obtain the Diffie-Hellman shared secret.
                // ------------------------------------------------------------
                ...
                final PublicKey aPK =
                        KeyFactory.getInstance("EC").generatePublic(new X509EncodedKeySpec(aEncoded));

                final KeyAgreement dh = KeyAgreement.getInstance("ECDH");
                dh.init(serverDhKP.getPrivate());
                dh.doPhase(aPK, true);
                final byte[] sharedSecret = dh.generateSecret();
